...
const initialState ={
  ...
  trigger: null,
  contactsSelected: [],
  elements:[{id: 1, name: 'startNode', parent: null,childOf:{number: 0},compo: <StartNode/>, children: []}],
  smsDetails:[],
  delay:[],
  condition: []
}
...
const workflowReducer = (state=initialState, action) => {
    switch (action.type) {
      ...
      return {
        ...state,
        elements: state.elements.filter( (_,i) => 
            i !== state.elements.length-1
        ),
    }
      ...
      return {
        ...
        elements: [...state.elements, action.add_yes_element,action.add_no_element,action.add_step_element],
      }
      default:
        return state;
    }
